...
def atualizar_grafo_e_buscas(
    ...
    n_clicks_carregar_grafo, n_clicks_salvar_grafo,  # Variável renomeada
    ...
    if trigger == 'toggle-directed-btn':
        if G.is_directed():
            # Cria um novo grafo não orientado
            G_novo = nx.Graph()
            # Adiciona nós e arestas do grafo orientado
            G_novo.add_nodes_from(G.nodes(data=True))
            for u, v, data in G.edges(data=True):
                # Adiciona a aresta com o peso
                G_novo.add_edge(u, v, weight=data.get('weight', 1))  # Usa peso padrão se não houver peso
            G = G_novo  # Atualiza G para o grafo não orientado
        else:
            # Cria um novo grafo orientado
            G_novo = nx.DiGraph()
            ...
            for u, v, data in G.edges(data=True):
                # Adiciona a aresta com o peso
                G_novo.add_edge(u, v, weight=data.get('weight', 1))  # Usa peso padrão se não houver peso
            G = G_novo  # Atualiza G para o grafo orientado
        grafo_atualizado = True

    if trigger == 'toggle-weight-btn':
        ponderado = not ponderado  # Alterna o status de ponderação
        for u, v, data in G.edges(data=True):
            if ponderado:
                # Define peso padrão se não houver peso
                if 'weight' not in data:
                    data['weight'] = 1
            else:
                if 'weight' in data:
                    del data['weight']  # Remove o peso
        grafo_atualizado = True
    ...
    # Adicionar aresta com nós selecionados
    if trigger == 'add-edge-btn':
        if selected_nodes and len(selected_nodes) == 2:
            u, v = selected_nodes[0]['id'], selected_nodes[1]['id']
            
            # Verifica se a aresta já existe, considerando a orientação
            if G.is_directed() and G.has_edge(u, v):
                return dash.no_update  # Não faz nada se a aresta já existe
            ...
            grafo_atualizado = True
    # Adicionar peso às arestas selecionadas
    ...
    if trigger == 'bfs-btn' and bfs_clicks:
        if selected_nodes and selected_nodes[0]['id'] in G:
            # Realiza a busca em largura a partir do nó selecionado
            bfs_result = list(nx.bfs_tree(G, selected_nodes[0]['id']))
            resultado_busca = f"BFS a partir de {selected_nodes[0]['id']}: {bfs_result}"
            grafo_atualizado = True  # Atualiza o grafo para refletir a nova informação
    elif trigger == 'dfs-btn' and dfs_clicks:
        if selected_nodes and selected_nodes[0]['id'] in G:
            # Realiza a busca em profundidade a partir do nó selecionado
    ...
    # Atualizar elementos do grafo e informações
        # Atualizar elementos do grafo e informações
    if grafo_atualizado:
        ...
        info = f"Grafo {'ponderado' if ponderado else 'não ponderado'}, " \
               ...
               f"\nVértices: {list(G.nodes)}" \
               f"\nArestas: {list(G.edges)}"
        if resultado_busca:  # Adiciona o resultado da busca se existir
            info += f"\n{resultado_busca}"  # Adiciona uma nova linha para melhor visualização
        return elementos, info, current_zoom

    return dash.no_update, dash.no_update, current_zoom
